...
Replace `board[m][n]` with `board[-1][-1]` on line 12.
</bug_fixes>
<bug_desc>
...
On line 12, board[m][n] seems to be indexing the bottom right value of the array. However, the indices are out of bounds, and therefore should be replaced to board[m-1][n-1] or board[-1][-1] to fix the mistake.
</bug_desc>
"""
class Solution:
